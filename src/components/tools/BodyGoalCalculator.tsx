...
import { ShieldCheck, Heart, Flame, Dumbbell, ShieldAlert, CheckCircle, Save, Info, Award, User, RefreshCw } from 'lucide-react';

interface BodyGoalCalculatorProps {
  onAssessmentSave: (metrics: AssessmentOutput & { inputs: AssessmentInputs }) => void;
}

export default function BodyGoalCalculator({ onAssessmentSave }: BodyGoalCalculatorProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Outputs Panel */}
      <div className="lg:col-span-7 bg-surface-container-high border border-outline/30 p-6 md:p-8 text-on-surface flex flex-col justify-between">
        {output ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="p-4 bg-surface-container-low border border-outline/10 text-center relative flex flex-col justify-between">
                ...
                <span className={`font-mono text-[8px] uppercase font-bold p-0.5 rounded-sm ${
                  output.bmiCategory.includes("Normal") ? "text-green-600 dark:text-green-400 bg-green-900/10 dark:bg-green-950/20" : "text-primary bg-primary/10"
